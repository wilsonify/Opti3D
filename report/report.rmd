...
In this project, we used FlashForge 3D printer to obtain a large cube as a STL file and pass it through a Slic3r to format it as a G-Code. We then model tensile strength from data obtain from Kaggle: [@okudan20183dprinterdataset]. After applying that model to our metadata, we model filament usage as a function of similar inputs. When we modeled strength, we found that the most important parameter was wall thickness. Extrusion temperature, and fill density were also significantly predictive of tensile strength. Every other parameter we looked at was discarded, the top ten features are summarized here.
